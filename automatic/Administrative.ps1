...
$GroupNames = @("Worker", "Direction", "Secretary", "Administrator")
...
try{
    ...
    # $ShareName = "\\scriptToServ\" + $ShareName 
...
$Users = @(
    @{Name="Worder"; Surname="Smith"; OU="Administration"; Group="Worker"},
    ...
    @{Name="Admin"; Surname="Williams"; OU="Workspace"; Group="Administrator"}
)
...
try {
    foreach ($GroupName in $GroupNames) {
        ...
        foreach ($Folder in $ReadFolders) {
            ...
            $rule = New-Object System.Security.AccessControl.FileSystemAccessRule($Group, "Read", "Allow")
        ...
        foreach ($Folder in $EditFolders) {
            ...
            $acl = Get-Acl -Path $Folder
            $rule = New-Object System.Security.AccessControl.FileSystemAccessRule($Group, "Modify", "Allow")
            $acl.SetAccessRule($rule)
            Set-Acl -Path $Folder -AclObject $acl
        ...
        foreach ($Folder in $CreateFolders) {
            ...
            $acl = Get-Acl -Path $Folder
            $rule = New-Object System.Security.AccessControl.FileSystemAccessRule($Group, "CreateFiles", "Allow")
            $acl.SetAccessRule($rule)
            Set-Acl -Path $Folder -AclObject $acl
        ...
        foreach ($Folder in $DeleteFolders) {
            ...
            $acl = Get-Acl -Path $Folder
            $rule = New-Object System.Security.AccessControl.FileSystemAccessRule($Group, "Delete", "Allow")
            $acl.SetAccessRule($rule)
            Set-Acl -Path $Folder -AclObject $acl
...
try {
    $screen = "\\scriptToServ\Shared\Screen.png"
    if (Test-Path $screen) {
...
$OpenOfficePath = "\\scriptToServ\Shared\OpenOffice 4\program\soffice.exe"
$OpenOfficeShortcutPath = "\\scriptToServ\Shared\OpenOffice 4\OpenOffice 4.1.15.lnk"
...
$SlackPath = "\\scriptToServ\Shared\Slack\slack.exe"
$SlackShortcutPath = "\\scriptToServ\Shared\slack\Slack.lnk"
try {
